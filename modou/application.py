"""稳定应用服务：UI、CLI 和 Agent 层唯一该调用的入口。

**为什么要有这一层。** 输入分层约束规定："不能让 UI 直接 import 内部引擎。"
`tools/run_one.py` 是一个单体流程，签名里带着 `instance_id`、`scaffold`、
`fixture_root` 这些只有 benchmark 才有的概念。任何直接调它的界面，
都会把 SWE-bench 的形状固化进产品。

这里定义三个入口，往后 UI 与 Agent 只认这三个：
...
**边界：本模块不做事实判定。** 它只负责把请求变成合法输入、选执行模式、
调引擎、把结果收成可序列化的句柄。判定在 `modou/`，证据在 `modou/ledger/`。
"""
...
class AnalysisRequest:
    ...
    # ---- Agent（Day 4 才真正接上；这里先进契约，免得以后改签名）
    goal: str = ""
    ...
    def resolve(self) -> ResolvedInput:
        ...
        if len(got) != 1:
            raise InputError(
                ...
                f"repo_path（你的仓库）/ fixture_root（自足样例）/ instance_id（冻结样本）")
        if self.repo_path:
        ...
        return inputs.from_benchmark(self.instance_id, self.scaffold or _default_scaffold())
...
@dataclass(frozen=True)
class RunEvent:
    """一条运行事件。Day 3 会让实时与回放共用它；现在只有回放这一侧。

    刻意不做成自由字典：UI「不读取任意日志猜状态，只根据结构化事件渲染」
    （事件渲染约束），字典会让这条纪律无声地退化。
    """
    kind: str
    run_id: str
...
def stream_events(source: "RunHandle | RunBundle") -> Iterator[RunEvent]:
    ...
    **当前是回放，不是实时。** 实时推送、中断恢复、SSE 都在 Day 3（WP2）。
    先把回放这一侧做出来，是因为事件溯源约束要求实时画面、离线回放和最终证书
    来自同一个真相源——那就必须先有源，再有推送。
    """
    rid = source.run_id
