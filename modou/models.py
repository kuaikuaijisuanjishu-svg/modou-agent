...
class TestVector:
    ...
    @classmethod
    def from_pairs(cls, pairs) -> "TestVector":
        """从账本 Fact 的 [[nodeid, status], ...] 还原向量。

        状态字面量必须已经是 TestStatus 的枚举值（"passed"，不是
        "PASSED"）：出现任何其他写法当场 ValueError。复验宁可扣留，
        也不把两套写法各自"规范化"后偷偷比成相等——那正是把整个
        复验判成"修订"的历史 bug。
        """
        return cls(tuple(sorted((str(t), TestStatus(s)) for t, s in pairs)))
...
class Admissibility(str, Enum):
    ...
    判据只用两样已经采集到的数据——逐项测试状态转变，和基线覆盖率里
    **这条测试**是否执行过被干预的行。后者需要 per-test 覆盖率上下文
    （``coverage`` 的 ``dynamic_context``），聚合覆盖率回答不了它。

    分级只会因为证据不足而下降，不会因为不确定而上升：映射不到上下文、
    上下文有歧义，一律记 B 而不是 A。
    """
    BEHAVIOURAL = "A"      # passed→failed，且这条测试在基线里执行过被干预的行
    INDIRECT = "B"         # passed→failed/error，但这条测试没有执行过那一行
    COLLATERAL = "C"       # passed→missing 或收集期错误：测试没有被执行
    ENVIRONMENTAL = "D"    # passed→skipped 或基线即 skipped：跳过条件变了


#: 只有这两级能支撑「承重」。C/D 不得签承重。
ADMISSIBLE = frozenset({Admissibility.BEHAVIOURAL, Admissibility.INDIRECT})

#: 由强到弱。一个单元取它最强的那一条。
ADMISSIBILITY_ORDER = (Admissibility.BEHAVIOURAL, Admissibility.INDIRECT,
                       Admissibility.COLLATERAL, Admissibility.ENVIRONMENTAL)
...
class EvidenceUnit:
    ...
    #: 本单元最强的一条可采性等级。None 表示还没定级。
    admissibility: Optional["Admissibility"] = None
    #: 逐条具名回归的定级：(test_id, 基线状态, 现在状态, 等级)。
    #: 证书据此点名，所以它必须与 ``regressions()`` 一一对应。
    graded_regressions: tuple[tuple[str, str, str, str], ...] = ()
    note: str = ""
    #: 账本里对应的实验与事实。M2 起由 TrialRunner 填——有了它，
    ...
    def statement(self) -> str:
        ...
        if self.verdict is Label.LOAD_BEARING:
            # 点名的必须是**可采**的那几条。把一条 C 级（根本没跑起来的）
            # 测试写进「依据」，措辞会和真实断言失败完全一样，而它什么都没
            # 约束——这正是这套分级要消掉的那张假证书。
            graded = [(t, g) for t, _, _, g in self.graded_regressions
                      if g in ("A", "B")] or [(t, "") for t, _, _ in self.regressions()]
            names = "、".join(t for t, _ in graded[:3]) or "（未记录）"
            more = f" 等 {len(graded)} 项" if len(graded) > 3 else ""
            if self.admissibility is Admissibility.INDIRECT:
                return (f"{where} 属于一个最小已隔离单元；移除该单元后，"
                        f"指定测试发生回归：{names}{more}；"
                        f"但这些失败的测试在基线中没有执行过这一行，"
                        f"因此只是间接证据")
            return (f"{where} 属于一个最小已隔离单元；移除该单元后，"
                    f"指定测试发生回归：{names}{more}")
