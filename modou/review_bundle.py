...
from .modes import (DEFAULT_DETERMINISTIC_SCHEDULER,
                    ISOLATION_MODES, execution_mode_for, product_mode_for)
...
_ID_KEYS = frozenset({"delivery_id", "evidence_run_id", "installation_id",
                      "repo_id", "repository_id", "run_id"})
# unit_id、review_id、event_id 不在 _ID_KEYS 里：三者都是服务端生成的
# 不透明十六进制（unit_id 更是 sha256(路径|类型|行号) 的单向哈希），
# 再脱敏不增加隐私，却会让账本 record_id（对 payload 内容寻址）和
# MCP 工具面（用原始 review_id 寻址）在公开投影后对不上——内容寻址
# 的证据链与稳定的寻址键，比重复遮蔽一个不可逆摘要更重要。
...
def _mode_fields(request: dict, provider: dict) -> tuple[str, str, str]:
    ...
    """
    # v2 起请求同时带 isolation_mode（隔离方式）与 legacy execution_mode
    # （历史上承载的也是隔离方式）。新包优先读新字段；旧包只有 legacy
    # 字段时，仅当它的值确实属于隔离词表才采信。
    legacy = str(request.get("execution_mode") or "")
    isolation = str(request.get("isolation_mode")
                    or (legacy if legacy in ISOLATION_MODES else "")
                    or "trusted_local")
    agent_level = str(request.get("agent_level") or "l1")
    provider_kind = str(provider.get("kind") or "deterministic")
    # 调度声明跟着"谁真的做了调度决策"走：live provider 不论 L1/L2 都会
    # 在 _next_action 里参与 continue/stop/重排，所以是 model 调度。
    # 标准审查即使跑在配置了模型的服务器上，调度也必须报告确定性
    # 策略：模式边界由 product_mode 决定，不由服务器配置决定。
    if (provider_kind != "deterministic"
            and str(request.get("product_mode") or "") != "standard"):
        scheduler = "model"
    else:
...
def build_review_bundle_v2(*, review_id: str, request: dict, plan: dict,
    ...
    # 投影会改写嵌套身份（event_id、run_id），但 v2 协议的确定性派生链
    # 必须在投影之后重建：事件 id 是 review_id:seq，账本 run_id 必须与
    # 顶层 evidence_run_id 同值。迁移旧包（bundle_v1_to_v2）做的是同一
    # 件事；真实新运行也要走同一条纪律，整包 verify 才验得过。
    evidence_run_id = _public_id(evidence_run_id, "evidence_run_id")
    for seq, event in enumerate(events, 1):
        if isinstance(event, dict):
            event["review_id"] = review_id
            event["event_id"] = f"{review_id}:{seq:06d}"
    if isinstance(evidence_bundle, dict):
        evidence_bundle["run_id"] = evidence_run_id
        for row in evidence_bundle.get("ledger") or []:
            if isinstance(row, dict):
                row["run_id"] = evidence_run_id
    execution_mode, scheduler_mode, isolation_mode = _mode_fields(request, provider)
    # 产品模式声明来自 request 本身；旧请求没有这个字段时由 provider
    # 推导。证据包不能声称比实际运行更强的模式边界。
    product_mode = str(request.get("product_mode")
                       or product_mode_for(str(request.get("model_provider")
                                               or "deterministic")))
