"""正式跑数的输出根：布局、不可覆盖协议、状态生命周期。

**run 1 的逐实例证书被 run 2 覆盖而永久丢失**（见 `runs/run1/MISSING.md`），
就是缺少这套协议的直接后果——当时没有任何东西阻止第二次跑数写进同一个目录。
所以这里的默认行为是拒绝，不是覆盖。

布局：

    <run_root>/
      freeze.json                 软件与输入的冻结
      source.tar
      evaluation_manifest.json    **不可变副本**，不是对 configs/ 的引用
...
**正式运行不在仓库内进行。** freeze 会往 run_root 写文件，而 `.gitignore`
不忽略 `runs/`，于是工作树变脏；而冻结校验又要求工作树干净——两者自我冲突。
正式跑数落 `~/.modou/formal-runs/<run_id>/`，跑完再把整个不可变 bundle
复制进仓库 `runs/` 归档，**复制之后不得改动内容**。
"""
...
#: 正式跑数的默认落点。刻意在仓库之外。
FORMAL_ROOT = Path(env_get("FORMAL_RUNS")
    ...
    def begin(self, *, cli: dict | None = None) -> "RunRoot":
        ...
        if self.official and self.has_products():
            raise RunRootBusy(
                ...
                f"\n这条协议是 run 1 证书被覆盖丢失之后补上的，不提供 --force。")
