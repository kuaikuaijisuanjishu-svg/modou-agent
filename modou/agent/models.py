...
是状态机安全约束明确要防的作弊。这里用 frozen dataclass + tuple 表达这条纪律，
...
class ActionKind(str, Enum):
    """模型动作的封闭词表。Day 4 只开放 CONTINUE / STOP。"""
...
#: 停滞判据窗口：连续 N 个观测指纹相同即 NO_PROGRESS。
#: 跑前冻结（2026-09-16，D1）。不许为了测试变绿调整这个数。
...
#: 初评开放的八个工具（封闭目录契约）。**这里没有的东西模型就调不到**——
...
# v2 control-plane actions are registered separately from repository-session
# handlers.  A model can propose these names, but only the ReviewManager owns
# their deterministic implementations and capability tokens.
CONTROL_ACTIONS_V2: tuple[ToolSpec, ...] = (
    ToolSpec("read_rules", ToolRisk.READ_ONLY, frozenset(), "读取已确认仓库规则"),
    ToolSpec("read_review_history", ToolRisk.READ_ONLY, frozenset(), "读取结构化历史摘要"),
    ToolSpec("revise_plan", ToolRisk.READ_ONLY, frozenset({"revision_sha256"}), "生成新计划版本"),
    ToolSpec("request_human_decision", ToolRisk.READ_ONLY, frozenset({"decision_id"}), "请求人工决策"),
    ToolSpec("generate_repair_candidate", ToolRisk.MUTATES,
             frozenset({"finding_ids_sha256"}), "生成封闭补丁候选工件"),
    ToolSpec("apply_repair_candidate", ToolRisk.MUTATES,
             frozenset({"patch_sha256"}), "在私有 worktree 应用候选"),
    ToolSpec("verify_repair_candidate", ToolRisk.EXECUTES_REPO,
             frozenset({"patch_sha256", "test_scope_sha256"}), "执行分层修复验证"),
    ToolSpec("deliver_local_branch", ToolRisk.MUTATES,
             frozenset({"patch_sha256"}), "创建唯一的本地分支和提交"),
    ToolSpec("propose_test", ToolRisk.MUTATES,
             frozenset({"finding_ids_sha256"}), "为无据新增行提议一条新测试并在隔离工作树验证"),
    ToolSpec("revise_test", ToolRisk.MUTATES,
             frozenset({"finding_ids_sha256"}), "读真实失败摘录修订测试提案（限 2 轮）"),
    ToolSpec("cancel_review", ToolRisk.READ_ONLY, frozenset(), "协作取消"),
    ToolSpec("archive_review", ToolRisk.READ_ONLY, frozenset(), "封存 Review"),
    ToolSpec("cleanup_review", ToolRisk.MUTATES, frozenset(), "清理精确 Review 临时资源"),
)

CATALOG_V2: tuple[ToolSpec, ...] = CATALOG_V1 + CONTROL_ACTIONS_V2


class ToolNotAllowed(ValueError):
    """模型请求了目录之外的工具，或给了未声明的参数。

    这是**请求**越权，不是**执行**越权——两者必须分开计数（控制面契约）。
...
class ToolCatalog:
    ...
    @classmethod
    def v2(cls) -> "ToolCatalog":
        return cls(CATALOG_V2)
...
class AgentAction:
    ...
    #: 展示给用户看的一句话理由。**不是思维链**——展示层协议要求它简短可展示。
    reason: str = ""
...
class Observation:
    ...
    @property
    def stagnation_fingerprint(self) -> tuple[str, str, bool | None, tuple[str, ...]]:
        """停滞判据的观测指纹：只含有界结构化事实。

        刻意排除 cost_s / evidence_id / observation_id——它们每步必然
        不同，掺进指纹会让判据在真正的故障循环上也永不触发。
        """
        return (self.anchor_id, self.status, self.identical_to_baseline,
                self.regressed_tests)
...
class AgentState:
    ...
    #: 它是 H1′ 的分母，动了它整个调度评测就失去意义（固定分母协议）。
    ...
    def stagnant(self) -> bool:
        """连续 NO_PROGRESS_WINDOW 个观测指纹相同即停滞。

        健康路径每步消耗一个锚点，指纹必然变化——判据在正常调度上
        永不触发。它挡的是无人值守下的真实故障面：观测回报的锚点与
        剩余集合不匹配（工具层故障/误报）时，advance() 移除不了任何
        候选，循环在同一处原地打转。
        """
        window = self.observations[-NO_PROGRESS_WINDOW:]
        if len(window) < NO_PROGRESS_WINDOW:
            return False
        return len({obs.stagnation_fingerprint for obs in window}) == 1
    ...
    def reorder(self, order: tuple[str, ...], observation_id: str = "") -> "AgentState":
        """Strict scheduler permutation: invalid requests are rejected, never repaired."""
        if self.agent_level not in {AgentLevel.L2, AgentLevel.L3}:
            raise ReprioritizationRejected(
                "AGENT_LEVEL_FORBIDDEN",
                "reprioritize requires server-bound L2 or L3")
