...
from ..ddmin import ProbeStrategy
from ..modes import PRODUCT_MODE_PROVIDER, PRODUCT_MODES, product_mode_for
...
class ReviewSpec:
    ...
    product_mode: str = "standard"
    probe_strategy: str = ProbeStrategy.HDD_INSPIRED.value
    ...
    def parse(cls, raw: Any) -> "ReviewSpec":
        ...
        allowed = {"instruction", "input_language", "source", "goal", "success_conditions",
                   ...
                   "created_at", "schema_version", "product_mode", "probe_strategy"}
        if set(raw) - allowed:
        ...
        autonomy, tests = AutonomyPolicy.parse(raw.get("autonomy_policy")), TestPolicy.parse(
            raw.get("test_policy"))
        # 产品模式是 v1 的顶层声明，与 autonomy_policy.model_provider 构成
        # 有意的双重声明：两处必须一致；缺省时从 provider 推导，旧调用方
        # 不破。显式不一致在 intake 就失败关闭，而不是替用户挑一处当真。
        product_mode = (str(raw.get("product_mode") or "")
                        or product_mode_for(autonomy.model_provider))
        if product_mode not in PRODUCT_MODES:
            raise ReviewSpecError("PRODUCT_MODE_INVALID", product_mode)
        if PRODUCT_MODE_PROVIDER[product_mode] != autonomy.model_provider:
            raise ReviewSpecError(
                "PRODUCT_MODE_PROVIDER_MISMATCH",
                f"{product_mode} requires {PRODUCT_MODE_PROVIDER[product_mode]}")
        # 最小化策略是执行层选项，不改变三态结论口径；v2 之前它只活在
        # v1 载荷里，两模式统一创建入口后必须随规格一起走，否则高级
        # 设置里的选择会变成装饰。取值封闭，能力门禁仍在 intake 复核。
        probe_strategy = str(raw.get("probe_strategy")
                             or ProbeStrategy.HDD_INSPIRED.value)
        if probe_strategy not in {item.value for item in ProbeStrategy}:
            raise ReviewSpecError("SPEC_PROBE_STRATEGY_INVALID", probe_strategy)
        ...
        return cls(instruction[:2000], ReviewSource.parse(raw.get("source")), goal,
                   ...
                   language, focus, created_at, product_mode=product_mode,
