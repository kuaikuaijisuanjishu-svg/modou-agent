...
class AnalysisSession:
    ...
    def _minimize(self, anchor_id: str, regressions: set[str]) -> None:
        ...
        not land in `Observation.cost_s`, which the cost oracle reads. Folding them
        ...
        def done(experiments: int = 0) -> None:
            ...
            certificate" are different findings, and the fixed-unit comparison
            ...
            if anchor_id not in self.drift_files:
                ...
                if allowed and pending:
                    u3, unresolved = hdd.run(
                        ...
                        contexts_by_line=self.cov.contexts.get(anchor_id, {}),
    ...
    def cleanup(self) -> None:
        """收掉这次分析的工作树。可重复调用，且**不会**盖掉正在传播的异常。

        `run_all` 在 `finally` 里调它。如果这里直接抛，一次失败的审查会
        变成一条"清理失败"的错误，真正的失败原因被丢掉——所以有异常在飞时
        只记下来，没有异常时才抛：残留目录是必须被看见的失败，不是可忽略项。
        """
        ...
        try:
            ...
            self.cleanup_error = f"{type(exc).__name__}: {exc}"[:300]
...
class ToolRouter:
    ...
    def execute_controlled(self, action, authority, *, repo_fingerprint: str):
        """Validate the complete v2 envelope and capability before dispatch.

        Kept separate from ``call`` because pre-approval repository discovery is
        deterministic controller work and has no frozen plan to bind yet.
        """
        action.validate(self.catalog)
        authority.verify(action, repo_fingerprint=repo_fingerprint)
        return self.call(action.tool, action.args)
