...
# schema 串的单一事实源：解析器、提示词与测试假 provider 一律 import 这两个
# 常量，不允许各自硬编码。曾有测试里的假 provider 把正确值写死、套件永远
# 绿而真实模型 100% 失败的事故（schema_version 盲区）——同一条串只允许
# 存在一份。元测试（tests/test_schema_literals.py）拦新出现的硬编码。
REPAIR_CONTEXT_SCHEMA_VERSION = "repair-context-v1"
REPAIR_CANDIDATE_SCHEMA_VERSION = "repair-candidate-v1"
...
class RepairCandidate:
    ...
    schema_version: str = REPAIR_CANDIDATE_SCHEMA_VERSION
    ...
    def parse(cls, raw: Any, context: RepairContext) -> "RepairCandidate":
        ...
        if raw.get("schema_version") != REPAIR_CANDIDATE_SCHEMA_VERSION:
