...
# v2（2026-09-16，D1）：停止规则全集加入 no_progress。
# 判据代际变化显式进版本号；已冻结计划从不重编译，不受影响。
PROMPT_VERSION = "planner-v1"
POLICY_VERSION = "probe-policy-v2"
...
class FrozenProbePlan:
    ...
    def as_dict(self) -> dict:
        out = {**self.plan.as_dict(), "plan_sha256": self.plan_sha256,
               "policy_version": self.policy_version}
        if self.review_spec_sha256:
            out["review_spec_sha256"] = self.review_spec_sha256
        return out
...
def compile_plan(draft: ProbePlanDraft, *, universe: tuple[str, ...],
                 user_budget_seconds: float, review_spec_sha256: str = "") -> FrozenProbePlan:
    ...
    if review_spec_sha256 and (len(review_spec_sha256) != 64
                               or any(c not in "0123456789abcdef" for c in review_spec_sha256)):
        raise PolicyError("REVIEW_SPEC_HASH_INVALID", "review spec hash must be sha256")
    signed = draft.as_dict()
    if review_spec_sha256:
        signed["review_spec_sha256"] = review_spec_sha256
    canonical = json.dumps(signed, ensure_ascii=False,
                           sort_keys=True, separators=(",", ":"))
    digest = hashlib.sha256(canonical.encode("utf-8")).hexdigest()
    return FrozenProbePlan(plan=draft, plan_sha256=digest,
                           review_spec_sha256=review_spec_sha256)
