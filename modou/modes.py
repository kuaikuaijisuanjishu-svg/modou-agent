...
#: the focus-specific orderings (coverage_first_order) can also apply
#: regression_first / coverage_gap_first, so they belong here too —
#: otherwise a legitimately sealed bundle fails its own vocabulary check.
SCHEDULER_MODES = frozenset({"model", "fifo", "coverage_first", "cost_first",
                             "regression_first", "coverage_gap_first"})
...
#:
#: The final provenance-bound scheduler rerun measured this on 30 patches:
#: coverage_first scored 0.6454 against FIFO's 0.5616 and the model's 0.5694,
#: and beat the model on all 18 eligible
#: tasks, and never once lost to FIFO. It costs no model call. See
#: the recorded scheduler decision — that is a post-hoc decision on the same
#: retrospective sample, not an independently validated
#: general result.
DEFAULT_DETERMINISTIC_SCHEDULER = "coverage_first"


#: The two product modes. 'standard' is the default and promises zero model
#: participation; 'agent' explicitly allows the constrained live model.
PRODUCT_MODES = frozenset({"standard", "agent"})

#: The provider each product mode must declare. A v1 request carries both
#: fields on purpose: two independent declarations that must agree, so a
#: stale or edited client cannot smuggle a live model into a standard review.
PRODUCT_MODE_PROVIDER = {"standard": "deterministic", "agent": "live"}


def product_mode_for(provider: str) -> str:
    """Derive the product mode from the declared provider.

    Used for callers that predate 'product_mode' (the v1 entry, migrated
    sessions): the mode is inferred deterministically, never asked of a
    model, and still passes the same intake cross-checks.
    """
    return "agent" if provider == "live" else "standard"
