"""M3：把引擎一直在算、却从没写进账本的观测落成 Fact。

**为什么这是必须的。** M2.1 之后账本里只有一种主张（`RequiredByTest`），
实测 511 行差分集里只有 **21 行（4.1%）**有真实 Claim 支撑，
其余 **95.9%** 全靠 `legacy.LegacyLine` 脚手架搬运。
脚手架只说明"旧引擎当时是这么标的"，不说明标注对不对——
在这个底座上做「证据编译」的叙述层，编译的其实是自己标的那份 JSON。

这里落五类事实。它们**都不是新算出来的**，全部来自引擎已有的中间结果：
