...
#: 只有 A（行为证据）和 B（间接证据）能支撑主张。与 ``models.ADMISSIBLE``
#: 同义，在这里重述一份是为了不让账本依赖 models。
ADMISSIBLE_GRADES = frozenset({"A", "B"})
_GRADE_ORDER = ("A", "B", "C", "D")


def _grade_of(graded, test_id: str) -> str:
    for tid, _, _, g in graded:
        if tid == test_id:
            return g
    return ""


def _best_grade(graded) -> str:
    present = {g for _, _, _, g in graded}
    for level in _GRADE_ORDER:
        if level in present:
            return level
    return ""
...
def required_by_test(items: list[Record], *, decision_id: str,
                     restored_fact_id: str, run_id: str,
                     graded_regressions: tuple[tuple[str, str, str, str], ...] = ()
                     ) -> Record:
    """从完整证据链生成 ``RequiredByTest``；任一条件不足即抛错。

    ``graded_regressions`` 是调用方给出的可采性定级。账本不重算它（定级需要
    per-test 覆盖率，账本里没有），但**要求它与账本自己推出的具名回归逐条对齐**，
    否则等级就成了一句没有回证的断言——那正是这套分级要消灭的东西。
    """
    ...
    graded = tuple(graded_regressions)
    if graded:
        derived = {(tid, before.value, after.value)
                   for tid, before, after in regressions}
        claimed = {(tid, before, after) for tid, before, after, _ in graded}
        if derived != claimed:
            raise ClaimDerivationError("可采性定级与账本推出的具名回归对不上")
        if not any(g in ADMISSIBLE_GRADES for _, _, _, g in graded):
            raise ClaimDerivationError(
                "没有任何一条可采证据（全部是 C 连带损坏或 D 环境变动）")
    ...
    return records.claim(
        ...
        data={"derivation_version": DERIVATION_VERSION,
              "regressions": [{"test_id": tid, "before": before.value,
                               "after": after.value,
                               "admissibility": _grade_of(graded, tid)}
                              for tid, before, after in regressions],
              "admissibility": _best_grade(graded),
              "restore_clean": True,
