"""M4：从账本**推导**逐行结论，不再搬运旧标签。

M1–M2.1 的账本里，逐行标签是 `legacy.LegacyLine` 脚手架——旧引擎标什么，
账本记什么。`projection_parity` 通过只说明"序列化没丢字段"。
这里换成真正的推导：**输入只有 Fact 与 Experiment，输出是逐行结论与 evidence_id。**
