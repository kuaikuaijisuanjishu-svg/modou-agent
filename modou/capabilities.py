...
_DENIAL_MARKERS = re.compile(
    ...
    r"不竞争|不等于|非目标|延期|暂不|范围外|排除|不承诺|待完成|尚未|拒绝|不采用|反例|"
    r"must not|never|do not|don't|forbidden|prohibited|out of scope")
#: A claim quoted in order to be measured as a misunderstanding is not a claim.
_MISCONCEPTION = re.compile(r"误解|误读|误认为|误以为|谣传|不实|以为")
...
def _denied(text: str, index: int) -> bool:
    ...
    guide. Two contexts count as a denial, and only two:
    ...
    if _MISCONCEPTION.search(text[index:line_end]):
        return True
...
def _asserted(pattern: re.Pattern, text: str) -> re.Match | None:
    ...
    for match in pattern.finditer(text):
        if not _denied(text, match.start()):
            return match
    return None
