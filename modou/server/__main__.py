"""CLI entry point: python -m modou.server --allow-repo ..."""
from __future__ import annotations

import argparse
import json
import secrets
from pathlib import Path

import uvicorn

from modou.agent.provider import OpenAICompatibleProvider, ProviderUnavailable
from modou.env import get as env_get

from .app import announcement_urls, create_app
from .control import RepoRegistry, ReviewManager
from .standing_authorization import (StandingAuthorization,
                                     StandingAuthorizationError)


def main() -> int:
    parser = argparse.ArgumentParser(description="水木验码本地 Review Cockpit")
    parser.add_argument("--allow-repo", action="append", required=True, type=Path,
                        help="明确允许执行测试的 Git 仓库（可重复）")
    parser.add_argument("--repo-python", action="append", default=[],
                        metavar="REPO=PYTHON",
                        help="为登记仓库绑定其测试解释器；浏览器不能覆盖")
    parser.add_argument("--host", default="127.0.0.1",
                        help="绑定地址：127.0.0.1 仅本机；0.0.0.0 允许局域网访问（展台三机）")
    parser.add_argument("--port", type=int, default=8765)
    parser.add_argument("--reviews-root", type=Path,
                        default=Path.home() / ".modou" / "reviews")
    parser.add_argument("--release-status", type=Path,
                        default=Path(__file__).resolve().parents[2] / "artifacts" /
                        "baseline" / "v02-release-status.json",
                        help="内部候选状态 JSON；缺失或无效时服务端 fail-closed")
    parser.add_argument("--preset-config", type=Path,
                        help="演示案例 JSON；仅接受已授权仓库名和仓库内相对测试路径")
    parser.add_argument("--agent-level", choices=["l1", "l2", "l3"], default="l1",
                        help="服务器绑定的 Agent 能力级别；浏览器不能覆盖")
    parser.add_argument("--standing-authorization", type=Path,
                        help="常驻授权文件（standing-authorization-v1）；"
                             "由人事先签发，仅 L3 自动批准路径消费，"
                             "加载失败即拒绝启动")
    parser.add_argument("--execution-mode", choices=["trusted_local", "sandboxed"],
                        default="trusted_local",
                        help="服务器绑定的测试子进程执行模式；浏览器不能覆盖")
    args = parser.parse_args()

    # Optional injection lets a local launcher restart the service without
    # losing the browser session. It is never accepted as a CLI argument (which
    # would expose it in the process list), and still defaults to 256 random bits.
    token = env_get("SESSION_TOKEN") or secrets.token_urlsafe(32)
    if len(token) < 32:
        parser.error("SHUIMU_YANMA_SESSION_TOKEN 至少需要 32 个字符")
    try:
        provider = OpenAICompatibleProvider.from_env()
    except ProviderUnavailable:
        provider = None
    python_by_repo = {}
    for binding in args.repo_python:
        if "=" not in binding:
            parser.error("--repo-python 必须是 REPO=PYTHON")
        repo_text, python_text = binding.split("=", 1)
        python_by_repo[Path(repo_text)] = Path(python_text)
    presets = []
    if args.preset_config:
        try:
            presets = json.loads(args.preset_config.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            parser.error(f"--preset-config 无法读取：{exc}")
        if not isinstance(presets, list):
            parser.error("--preset-config 顶层必须是数组")
    registry = RepoRegistry(args.allow_repo, python_by_repo=python_by_repo,
                            presets=presets)
    standing_authorization = None
    if args.standing_authorization:
        try:
            standing_authorization = StandingAuthorization.from_path(
                args.standing_authorization)
        except StandingAuthorizationError as exc:
            parser.error(f"--standing-authorization 无法加载（{exc.code}）："
                         f"{exc.detail}")
    manager = ReviewManager(registry, root=args.reviews_root, provider=provider,
                            agent_level=args.agent_level,
                            execution_mode=args.execution_mode,
                            standing_authorization=standing_authorization)
    web_dist = Path(__file__).resolve().parents[2] / "web" / "dist"
    app = create_app(manager=manager, token=token, host=args.host,
                     port=args.port, web_dist=web_dist,
                     release_status_path=args.release_status)
    # flush 是必需的：stdout 被重定向（`> log`、nohup、后台跑）时不是 tty，
    # Python 会缓冲；而 uvicorn.run() 之后就再也不返回了，于是这三行——
    # 包括**唯一一次打印的 token**——永远进不了日志文件。
    print(f"执行模式：{args.execution_mode}（服务器绑定）", flush=True)
    print(f"Agent level：{args.agent_level.upper()}（服务器绑定）", flush=True)
    if standing_authorization is not None:
        print(f"常驻授权：{args.standing_authorization}"
              f"（sha256:{standing_authorization.source_sha256[:12]}，"
              f"授权人 {standing_authorization.authorized_by}）", flush=True)
    for url in announcement_urls(args.host, args.port):
        print(f"打开：{url}/#token={token}", flush=True)
    uvicorn.run(app, host=args.host, port=args.port, access_log=False)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
