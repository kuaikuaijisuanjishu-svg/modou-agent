...
def build_model(lines: list[LineResult], units: list[EvidenceUnit],
    ...
    """
    # 可采性等级跟着所属证据单元走：工作台悬浮要显示可采性 A/B，
    # 模型必须自带等级，而不是让前端从证书措辞反推。未定级为 None，
    # 宁可缺省也不编造等级。
    unit_grade = {u.unit_id: u.admissibility for u in units}
    return {
        ...
        "lines": [
            {
                ...
                "unit_id": r.unit_id,
                "admissibility": (unit_grade.get(r.unit_id).value
                                  if unit_grade.get(r.unit_id) else None),
        ...
        "units": [
            {
                ...
                "admissibility": (u.admissibility.value
                                  if u.admissibility else None),
