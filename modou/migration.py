...
def matrix_measurements_v1_to_v2(measurements: dict) -> dict:
    """Migrate the real round-1 execution receipts, not just the manifest.

    The seal manifest that points at these rows was already migrated, but the
    rows themselves — the actual per-repository measurements — were not, so
    nothing carried them into the v2 world where they could be counted or
    contradicted.  Every row is preserved verbatim and the per-status tally is
    written alongside, so a later reader can check that the migration neither
    dropped a repository nor invented one.
    """
    if detect_schema(measurements) != "public-repo-matrix-measurements-v1":
        raise MigrationError("expected public-repo-matrix-measurements-v1")
    rows = list(measurements.get("measurements") or [])
    if not rows:
        raise MigrationError("measurement receipt carries no rows to migrate")
    tally: dict[str, int] = {}
    adapters: dict[str, int] = {}
    for row in rows:
        tally[str(row.get("status") or "UNKNOWN")] = tally.get(str(row.get("status") or "UNKNOWN"), 0) + 1
        adapters[str(row.get("adapter") or "unknown")] = adapters.get(str(row.get("adapter") or "unknown"), 0) + 1
    repositories = [str(row.get("repo_id") or "") for row in rows]
    return {"schema_version": "public-repo-matrix-measurements-v2",
            "sample_scope": "legacy_20_repo_matrix",
            "candidate_commit": measurements.get("candidate_commit", ""),
            "generated_at": measurements.get("generated_at", ""),
            "machine_gate_eligible": False,
            "consistency_definition": "legacy round-1 execution rows; no v2 product-off/product-on"
                                      " conclusion and no fixed-six denominator is derivable from them",
            "conservation": {"source_row_count": len(rows), "migrated_row_count": len(rows),
                              "status_counts": dict(sorted(tally.items())),
                              "adapter_counts": dict(sorted(adapters.items())),
                              "distinct_repositories": len(set(repositories)),
                              "duplicate_repository_rows": len(repositories) - len(set(repositories))},
            "measurements": copy.deepcopy(rows),
            "sources": list(measurements.get("sources") or []),
            "migration": {"source_schema": "public-repo-matrix-measurements-v1",
                           "source_preserved": True, "lossless": True},
            "legacy_measurements": copy.deepcopy(measurements)}


def matrix_measurements_v2_to_v1(measurements: dict) -> dict:
    """Restore the preserved source byte-for-byte in value terms."""
    if detect_schema(measurements) != "public-repo-matrix-measurements-v2":
        raise MigrationError("expected public-repo-matrix-measurements-v2")
    legacy = measurements.get("legacy_measurements")
    if not isinstance(legacy, dict) or detect_schema(legacy) != "public-repo-matrix-measurements-v1":
        raise MigrationError("v2 measurements lack their preserved v1 source")
    return copy.deepcopy(legacy)


def matrix_remote_summary_v1_to_v2(summary: dict) -> dict:
    """Migrate the round-1 evaluation receipt with its incompleteness intact.

    The summary's own conclusion is INCOMPLETE.  Migration must not quietly
    upgrade that into a v2 evaluation, so the v2 form carries the blocking
    reasons and claim boundary forward and stays ineligible for the gate.
    """
    if detect_schema(summary) != "public-repo-matrix-remote-summary-v1":
        raise MigrationError("expected public-repo-matrix-remote-summary-v1")
    conclusion = str(summary.get("conclusion") or "")
    blocking = list(summary.get("blocking_reasons") or [])
    if conclusion != "INCOMPLETE" and not blocking:
        raise MigrationError("a complete round-1 summary would need re-execution, not migration")
    return {"schema_version": "public-repo-matrix-evaluation-v2",
            "sample_scope": "legacy_20_repo_matrix",
            ...
            "formal": {"required_runs": 2, "compatible": 0, "collected": 0,
                        "minimum_compatible": 0},
            "evaluation_errors": ["legacy_round1_summary_incomplete", *blocking],
            "claim_boundary": summary.get("claim_boundary", ""),
            "known_exclusions_preserved": [summary.get("notable_exclusion")]
            if summary.get("notable_exclusion") else [],
            "migration": {"source_schema": "public-repo-matrix-remote-summary-v1",
                           "source_preserved": True, "source_conclusion": conclusion,
                           "incompleteness_preserved": True},
            "legacy_summary": copy.deepcopy(summary),
            "status": "METHOD_INCOMPLETE"}
