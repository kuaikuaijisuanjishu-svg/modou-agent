...
M2 起，「删除 → 跑 → 一律回滚」由 `modou/trial.py` 的 TrialRunner 统一执行，
...
from ..models import (ADMISSIBLE, Admissibility, EvidenceUnit, Label,
                      TestVector, Transform, Unlabeled, make_unit_id)
...
def run(ws, adapter, *, path: str, added: set[int], pending: set[int],
        ...
        contexts_by_line: dict[int, frozenset[str]] | None = None
        ) -> tuple[list[EvidenceUnit], dict[int, Unlabeled]]:
    """对一个文件跑层次化探测。

    pending：还需要归因的行（被执行过、暂时挂 NOT_ISOLATED 的）。
    contexts_by_line：基线里每一行被哪些测试执行过（per-test 覆盖率上下文）。
    没有它只能区分「向量变了」和「向量没变」，区分不了「这条失败的测试真的
    看见过这一行」和「它只是被连带崩掉了」。
    返回 (证据单元, {行号: 未标注原因})。
    """
    contexts_by_line = contexts_by_line or {}
    ...
    while queue and remaining():
        ...
        # 在签任何东西之前先定级——「向量变了」不等于「这段代码受约束」。
        graded = adm.grade(vector.regressions(baseline),
                           deleted_lines=m.deleted,
                           contexts_by_line=contexts_by_line)
        unit.graded_regressions = graded
        unit.admissibility = adm.best(graded)

        if unit.admissibility not in ADMISSIBLE:
            # 全部是 C 或 D：测试要么根本没执行（import 崩了、收集期错误），
            # 要么只是跳过条件变了。两种都不是行为约束，不得签承重。
            # 这恰恰是应该触发自动补测的信号：这一行的行为，当前测试范围里
            # 没有任何断言看得见。
            reason = (Unlabeled.COLLATERAL_BREAKAGE
                      if unit.admissibility is Admissibility.COLLATERAL
                      else Unlabeled.ENVIRONMENT_SHIFT)
            unit.verdict = Label.UNLABELED
            for ln in set(node.span) & remaining():
                unresolved[ln] = reason
                resolved.add(ln)
