...
def test_repo_snapshot_tracks_exact_untracked_paths_and_preserves_ascii_digest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        _git(repo, "add", "-A")
        _git(repo, "commit", "-qm", "base")
        (repo / "b.py").write_text("old ASCII content\n")
        legacy = hashlib.sha256()
        legacy.update(run_git(["diff", "--binary", "HEAD"], cwd=repo, check=True).stdout.encode())
        legacy.update(b"b.py")
        legacy.update(hashlib.sha256((repo / "b.py").read_bytes()).digest())
        assert _repo_snapshot(repo)["content_sha256"] == legacy.hexdigest()
        for name in ['中文文件.py', 'quote"file.py', 'line\nbreak.py', ' leading.py ']:
            path = repo / name
            path.write_text("first\n")
            before = _repo_snapshot(repo)
            path.write_text("second\n")
            after = _repo_snapshot(repo)
            assert before["status_sha256"] == after["status_sha256"], name
            assert before["content_sha256"] != after["content_sha256"], name
            assert before["snapshot_sha256"] != after["snapshot_sha256"], name
            path.write_text("third\n")
            assert _repo_snapshot(repo)["snapshot_sha256"] != after["snapshot_sha256"], name
