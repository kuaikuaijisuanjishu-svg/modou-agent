"""生成自足 demo 仓库：一个普通 Python 项目 + 一份"AI 刚写完、还没提交"的改动。

**和 `tests/fixtures/four_state_repo` 的区别不是内容，是入口。**
fixture 走 `fixture_root`，仍然带着 benchmark 的形状（要有 meta、要有
instance_id）。这个 demo 走的是真实用户路径：
...
BASE = {
    ...
    ".shuimu/review-memory.yaml": json.dumps({
        ...
        "records": [{
            ...
            "applies_to": ["tests/test_backoff.py"],
            "kind": "test_convention",
            "source_review_id": "review-mem-seed",
            "evidence_ids": ["claim-backoff-1"],
